...
**SMS dispatch** — the SMS counterpart to **mail dispatch**, same shape and same posture: a background task (Twilio-backed), bounded retry, best-effort delivery, failures to application logs only. **It knows nothing about why a message is sent**, same as mail dispatch. Its only consumer today is **Notification**, via the same channel ceiling email uses — no caller passes a channel meaning "SMS but not email." Distinct from phone verification (Authentication), which calls it directly to deliver a one-time code, the same way Authentication consumes mail dispatch directly for password reset without depending on Notification. See `docs/superpowers/specs/2026-07-26-notifications-email-sms-design.md`.
