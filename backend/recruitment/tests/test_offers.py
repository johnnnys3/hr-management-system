...
class OfferLetterTests(APITestCase):
    ...
    def test_hr_administrator_can_issue_an_offer_and_a_document_is_generated(self):
    ...
    def test_hr_administrator_can_record_a_decision(self):
        self.client.force_authenticate(self.hr_admin)
    ...
    def test_hr_officer_can_read_but_not_issue_or_decide_offers(self):
        ...
        `docs/07-iam-rbac.md` §4.2 grants HR Administrator, not HR Officer, C/U (absorbed from Recruiter, retired — ADR-0013)."""
        self.client.force_authenticate(self.hr_admin)
        create_response = self.client.post(
            f'/api/applications/{self.application.pk}/offer/', {'offered_salary': '95000.00'},
        )
