...
class PayGradeTests(APITestCase):
    ...
    def test_hr_administrator_read_includes_salary_figures(self):
        """Recruiter's blind-selection carve-out (issue #98) is retired
        with the role, ADR-0013 — HR Administrator, which absorbs it,
        already has full compensation access and needs no blind view."""
        pay_grade = make_pay_grade(name='Level 3')
        self.client.force_authenticate(user_with_role('hra2@example.com', HR_ADMINISTRATOR))

        response = self.client.get('/api/pay-grades/')

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0]['min_salary'], '1000.00')
        self.assertEqual(response.data[0]['max_salary'], '2000.00')
