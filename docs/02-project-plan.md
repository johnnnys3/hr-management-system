...
| 5 | Dashboard | SRS §3.1 role-specific dashboards |
| 6 | Employee Management | HRMS-FR-001 to HRMS-FR-012, HRMS-BR-001 to HRMS-BR-003, HRMS-DR-001 to HRMS-DR-004, HRMS-DR-007, HRMS-DR-008, HRMS-DR-010, HRMS-NFR-007, HRMS-NFR-008 |
| 7 | Departments | HRMS-BR-002; SRS §2.7 HR configuration |
| 8 | Reporting Structure | HRMS-FR-008, HRMS-BR-004 |
| 9 | Recruitment | HRMS-FR-013 to HRMS-FR-021 |
| 10 | Onboarding | HRMS-FR-022 to HRMS-FR-024, HRMS-BR-013 |
| 11 | **Notification** | **HRMS-FR-033, HRMS-FR-034 (SRS §4.3, self-service); the SRS §3.4 in-app feed of pending tasks and request updates. Consumes module 2 for its email channel. Moved from 8 at v1.4; ADR-0011** |
| 12 | Employee Self-Service | HRMS-FR-025 to HRMS-FR-029, HRMS-FR-044, HRMS-NFR-017, HRMS-BR-006 |
| 13 | Manager Self-Service | HRMS-FR-030 to HRMS-FR-032, HRMS-NFR-018, HRMS-BR-007 |
| 14 | Leave Management | HRMS-FR-063 to HRMS-FR-071, HRMS-BR-009 to HRMS-BR-011, HRMS-DR-006 |
...
| 7 | 2026-09-01 | M4 factor recovery and administration prohibition (1d of 4); M5 Dashboard (1.5d); M6 Employee Management (2.5d of 4) |
| 8 | 2026-09-08 | M6 Employee Management (1.5d of 4); M7 Departments (1d); M8 Reporting Structure (1.5d); M9 Recruitment (1d of 4) |
| 9 | 2026-09-15 | M9 Recruitment (3d of 4); M10 Onboarding (2d) |
| 10 | 2026-09-22 | M11 Notification (2d); M12 Employee Self-Service (2.5d); M13 Manager Self-Service (0.5d of 2) |
| 11 | 2026-09-29 | M13 Manager Self-Service (1.5d of 2); M14 Leave Management (3.5d of 4) |
| 12 | 2026-10-06 | M14 Leave Management (0.5d of 4); M15 Compensation and Benefits (4d); M16 Payroll (0.5d of 8) |
...
| M5 Dashboard | 1.5 | |
| M6 Employee Management | 4 | 12 FRs, document upload, audit history. **Unchanged at v1.3 and v1.4.** HRMS-FR-010's audit history is a filtered read of M1's store, not a second store; there was no separable audit line here to move to module 1. See §6.1 |
| M7 Departments | 1 | |
| M8 Reporting Structure | 1.5 | |
| M9 Recruitment | 4 | 9 FRs, requisition approval, interview scheduling, offer generation. **Consumes neither M2 nor M11**: HRMS-FR-014 requires an approval status and no notification, and no requirement announces a requisition decision. See §6.1 |
| M10 Onboarding | 2 | Email-only per SRS §3.4; consumes M2, not M11 |
| M11 Notification | 2 | HRMS-FR-033, HRMS-FR-034 (SRS §4.3); the SRS §3.4 in-app feed of pending tasks and request updates; delivery by email through M2. **New at v1.4, and not a re-estimate of the 1.5 days v1.1 gave a module this replaces** — that figure costed a different thing in a different place. See §7.3 and ADR-0011 |
| M12 Employee Self-Service | 2.5 | |
| M13 Manager Self-Service | 2 | |
| M14 Leave Management | 4 | 9 FRs, balance arithmetic, policy enforcement, calendar |
...
| TBD-007 | HR approval workflows | **Open. Not resolvable by this project.** SRS §2.7 assumes HR defines them | `docs/05-database-schema.md` (approval records); M9 Recruitment (HRMS-FR-014); M14 Leave Management (HRMS-BR-009). Also M11 Notification, which fires on the approval events HRMS-FR-033 and HRMS-FR-034 name. Also M4, whose HRMS-NFR-024 recovery flow requires an approver who administers no accounts — the approval is defined by the requirement, but who holds it is not | **Before M3 sign-off, 2026-07-23.** The schema must model approval whether or not the workflow content is known |
| TBD-008 | Leave policy rules | **Open. Not resolvable by this project.** SRS §2.7 assumes HR defines them | M14 Leave Management: HRMS-FR-067, HRMS-FR-068, and the "unless policy allows" condition on both | Before M14 begins, 2026-09-23 |
| TBD-009 | Document retention policy | **Open. Not resolvable by this project** | `docs/09-deployment-plan.md`; audit log retention per `docs/07-iam-rbac.md` §7.3; M6 document storage per ADR-0007 | Before Stage 9 |
...
| TBD-012 | External job board integration | **Open** | M9 Recruitment: HRMS-FR-015. SRS §2.5 notes external posting may depend on third-party API availability | Before M9 begins, 2026-09-07 |
...
| TBD-014 | Reporting dashboard KPIs | **Open. Not resolvable by this project** | M5 Dashboard; M17 Reports: HRMS-FR-049 to HRMS-FR-054 | Before M5 Dashboard begins, 2026-08-26 |
...
| The build order is revised after development starts | Capacity spent on modules a revision reorders | The §7.5 option decision is taken, and **the last unconfirmed placement is resolved at v1.4**: §11 question 2 is closed, the owner having confirmed the split, M2 Mail dispatch, M11 Notification, and both estimates on 2026-07-15 (§6.1, ADR-0011). No placement in §6.1 is now the plan's own inference. **The risk is not thereby retired**: the module-8 placement was confirmed by nobody and survived three versions because its reason was never checked against the SRS, and what closes it here is a reading of SRS §3.3, §3.4, and §4.3 — not the confirmation. `docs/04-system-architecture.md` records the assignment at M2, 2026-07-21; it does not reopen it |
