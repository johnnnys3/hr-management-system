...
| 14 | **Dashboard** | **SRS §3.1 role-specific dashboards. Reads modules 10 and 13 (`docs/06-api-contracts.md` §4.10); moved from 5 at v1.5 on the provider-precedes-consumer rule this plan has applied since v1.1. Its Executive aggregate view still precedes module 17 Reports, and that residual is recorded below rather than resolved by a further move** |
...
**v1.5 relocates Dashboard's 1.5 days from week 7 to weeks 11 and 12, and every module between the two positions absorbs the difference.** Removing Dashboard from week 7 leaves that week 1.5 days under capacity, which pulls every later module's start 1.5 days earlier until Dashboard is reinserted after Leave Management; from Compensation and Benefits onward the allocation is arithmetically identical to v1.4, because the 1.5 days removed at week 7 and the 1.5 days reinserted at weeks 11–12 net to zero over the intervening span. **No module's total days change, no week's capacity is exceeded, and weeks 13 to 17 are unchanged from v1.4 to the day** — the table below them was not re-derived; it is the same table, because the arithmetic that produces it does not depend on where within weeks 7 to 12 the same 49.5 days of work fall.
...
| M14 Dashboard | 1.5 | SRS §3.1 role-specific dashboards; a read-only composite over M10 and M13 per `docs/06-api-contracts.md` §4.10, no table of its own. **Moved from module 5 at v1.5**, on the provider-precedes-consumer rule: at module 5 it preceded both modules it reads. See §6.1 |
...
| The build order is revised after development starts | Capacity spent on modules a revision reorders | The §7.5 option decision is taken, and **the last unconfirmed placement is resolved at v1.4**: §11 question 2 is closed, the owner having confirmed the split, M2 Mail dispatch, M10 Notification, and both estimates on 2026-07-15 (§6.1, ADR-0011). No placement in §6.1 is now the plan's own inference. **The risk is not thereby retired**: the module-8 placement was confirmed by nobody and survived three versions because its reason was never checked against the SRS, and what closes it here is a reading of SRS §3.3, §3.4, and §4.3 — not the confirmation. `docs/04-system-architecture.md` records the assignment at M2, 2026-07-21; it does not reopen it |
