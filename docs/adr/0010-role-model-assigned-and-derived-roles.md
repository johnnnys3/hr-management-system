...
- The privileged role has a bounded, stated scope consistent with HRMS-NFR-019, and the two routes by which it could have reached payroll data unilaterally — `is_superuser`, and self-grant — are both closed by construction rather than by instruction.
...
- The credential-reset route to payroll data is detected and not prevented, and cannot be closed without an SRS revision. See the rationale above.
