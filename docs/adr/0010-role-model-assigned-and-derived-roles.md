...
- The privileged role has a bounded, stated scope consistent with HRMS-NFR-019, and the routes by which it could have reached payroll data unilaterally — `is_superuser`, self-grant, the proxy account, and the credential reset — are closed by construction rather than by instruction, the last of them by HRMS-NFR-024 as of SRS v1.1. What survives is one narrow opening, stated in the rationale above rather than left to be discovered.
