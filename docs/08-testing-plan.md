...
| John Kessie | 2026-07-23 | **Every reference to pytest-django and factory_boy replaced with actual practice** (Django's built-in test runner and plain ORM `.create()`/`.bulk_create()` fixture helpers), found during the same documentation audit that corrected `docs/03-tech-stack.md` §9 (now v1.2, DOC-018). §1.3, §3.1, §3.2, §3.4, §5, §7.1 all cited the never-installed tools; none of the underlying strategy, obligations, or control descriptions change — this is a tool-name correction throughout, not a re-design. Filed as DOC-018 | 1.5 |
| John Kessie | 2026-07-23 | **§7.1's NFR timing mechanism and §3.4's database-constraint control are now built** (`backend/contracts/tests/test_nfr_timing.py`, `test_db_constraints.py`). Timing: one test per HRMS-NFR-001 to -004, wall-clock time against a 200-employee fixture this project generates, printed alongside the fixture size per §7.1's own instruction — asserts the mechanism exists and produces a number, nothing about a threshold, since TBD-016/017 remain open. HRMS-NFR-005 is not duplicated here; payroll's own async poll-to-completion tests already time it per run. Constraints: building the completeness sweep §3.4 calls for found several HRMS-DR-005 tables (`pay_grade`, `offer_letter`, `bonus_award`, `allowance_type`, `employee_allowance`, `benefit`, `payslip`, `payslip_line`) had only serializer-level (400) coverage, never a direct-database-bypass test proving the `CHECK` constraint itself holds — and that HRMS-DR-001 (`employee.employee_number`) and HRMS-DR-002 (`user_account.email`) had no direct-bypass test at all. All now covered; HRMS-DR-006 and the two named self-approval `CHECK`s already had this coverage and are not duplicated. Closes M18's last two DOC-017 line items | 1.4 |
...
- **`docs/03-tech-stack.md`** (v1.2) §9 — Django's built-in test runner (`manage.py test`), plain ORM `.create()`/`.bulk_create()` fixtures, Vitest with React Testing Library, Playwright, and drf-spectacular. Settled tool selections this document builds a strategy on top of, not chooses among.
...
**Tool: Django's test runner** (`docs/03-tech-stack.md` §9). **Catches: an endpoint that omits its visibility rule** (plan §9; ADR-0005).
...
1. **Existence and shape.** For every endpoint `docs/06-api-contracts.md` §4 lists, a contract test (Django's test runner, using DRF's `APIClient` against the URL and method named) asserts the endpoint exists, accepts the method(s) listed, and its generated schema's request/response shape matches the resource fields §4's row names. This is not a hand-maintained OpenAPI diff; it is a test suite that imports the generated schema (`drf-spectacular`'s `SchemaGenerator`) and asserts against it, so the check runs against what DRF actually serves rather than against a document a developer might forget to update.
...
**Tool: plain ORM `.create()`/`.bulk_create()` helpers** (`docs/03-tech-stack.md` §9), one helper per model, composed rather than duplicated across test modules.

### 5.1 Ordinary Fixtures

Every table in `docs/05-database-schema.md` §4's entity catalog has a corresponding helper producing a valid row against that table's constraints (§6 there), built with explicit field values (no faker library is used) by default and explicit overrides where a test's assertion depends on a specific value (a `base_salary` a payroll test computes PAYE against, an `employment_status` a visibility test depends on). Helpers compose by constructing their own foreign-key rows directly (an employee-fixture helper builds its own department row unless one is supplied), and this document does not restate that pattern per model.

### 5.2 Multi-Actor Fixtures

Several constraints identified in §3 and §4 above cannot be exercised by a single-user fixture, because the constraint's entire point is that two distinct identities are involved. These are named once here rather than per module:

- **Payroll approver ≠ initiator** (`docs/05-database-schema.md` §3.5). A shared helper produces a run with a distinct `initiated_by` and a distinct candidate `approved_by`, both Payroll Officer role holders, so that tests asserting the *rejection* of self-approval and tests asserting the *success* of a legitimate approval draw from the same fixture shape rather than two ad hoc setups that could silently drift apart.
...
- **Leave request across employee and manager** (HRMS-BR-009 to HRMS-BR-011). An employee/manager fixture pair sharing a `reporting_relationship` row, used by every module 13 test that asserts an approval or rejection outcome, so the balance-decrement and no-decrement assertions run against the same two-identity shape.
...
**What is not built without a tool selection this document does not make.** HRMS-NFR-006's concurrent-load condition — 200 simultaneous users sustaining HRMS-NFR-001 to HRMS-NFR-005 — requires a load-generation tool that issues concurrent requests at scale, and `docs/03-tech-stack.md` §9's QA tooling table names none: Django's test runner, its ORM fixture helpers, Vitest/RTL, Playwright, and drf-spectacular each verify correctness or a single request's behaviour, not concurrent throughput. This document does not select one. Introducing a load-testing tool is the kind of technology decision `docs/03-tech-stack.md` made for every other layer of this system, and this document is not the place to make it by implication — the same reasoning that document itself gives for declining to invent things below its altitude (`docs/06-api-contracts.md` §7's second-factor-mechanism item is the precedent). **This is recorded as an open item (§9 below) rather than resolved here**, and HRMS-NFR-006 is not verifiable by any test this document specifies until it is.
