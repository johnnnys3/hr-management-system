...
| John Kessie | 2026-07-27 | **Recruiter retired, merged into HR Administrator** (SRS v1.5, ADR-0013 amending ADR-0010). §4.6's Recruiter-named rows renamed to HR Administrator; `IsRecruiter` is `CanWriteRecruitment` in `backend/recruitment/permissions.py`. §4.6's approve row gains `self_approval_forbidden`: HR Administrator now both creates and approves requisitions, so `/api/job-requisitions/{id}/approve/` rejects a caller who is the requisition's own `requested_by`, backed by `docs/05-database-schema.md` §4.7's new `CHECK` constraint — same shape as `/api/payroll-runs/{id}/approve/`'s existing self-approval rejection. §4.4's departments row and §4.7's onboarding-checklist rows drop Recruiter (HR Administrator's existing access covers it). §4.13's pay-grades row drops Recruiter's blind-selection carve-out (v1.6) — HR Administrator, which absorbs it, already has full read | 2.0 |
...
| Endpoint | Method | Permission | Visibility | Notes |
|---|---|---|---|---|
| `/api/departments/` | GET | HR Administrator: R; HR Officer, Payroll Officer: R (`docs/07-iam-rbac.md` §4.2's HR configuration row) | Organisation-wide; no per-role scoping — departments are reference data, not employee-scoped | |
...
Recruiter is retired, merged into HR Administrator (ADR-0013, SRS v1.5) — every row below that named Recruiter now names HR Administrator instead; HR Officer's access is unchanged.

| Endpoint | Method | Permission | Visibility | Notes |
|---|---|---|---|---|
| `/api/job-requisitions/` | GET, POST | HR Administrator: C, R (`docs/07-iam-rbac.md` §4.2) | HR Administrator: full (`docs/07-iam-rbac.md` §5 — no explicit requisition row; inherits Recruitment module's "HR Administrator: full, others: none" from `docs/04-system-architecture.md` §4's module 8 row) | |
| `/api/job-requisitions/{id}/` | GET, PATCH | HR Administrator: R, U | HR Administrator: full | |
| `/api/job-requisitions/{id}/approve/` | POST | HR Administrator: A (`docs/07-iam-rbac.md` §4.2's "Requisition approval" row) | n/a | Sets `status = 'approved'`, `approved_by`. Distinct endpoint per §2.9, since approval is a gated transition, not an ordinary field write. **Rejected, `403` with `code: "self_approval_forbidden"`, where the caller is the requisition's `requested_by`** — enforced first by `docs/05-database-schema.md` §4.7's `CHECK (approved_by IS NULL OR approved_by <> requested_by)` at the database layer, restated here at the API layer as the first check this endpoint performs. New with ADR-0013: HR Administrator now both creates and approves requisitions, where Recruiter and HR Administrator previously held those two functions apart |
| `/api/job-requisitions/{id}/reject/` | POST | HR Administrator: A | n/a | Sets `status = 'rejected'`. Not self-approval-checked — rejecting one's own requisition is not the conflict-of-interest concern the approve endpoint's check exists for |
| `/api/job-postings/` | GET, POST | HR Administrator: C, R; HR Officer: R (`docs/07-iam-rbac.md` §4.2) | HR Administrator: full; HR Officer: read | `POST` requires `requisition.status = 'approved'` — enforced in application code, not a schema constraint (`docs/05-database-schema.md` §4.7 states no such check) |
| `/api/job-postings/{id}/` | GET, PATCH | HR Administrator: R, U; HR Officer: R | HR Administrator: full; HR Officer: read | |
| `/api/job-postings/{id}/publish/` | POST | HR Administrator: U | n/a | Sets `published_at` |
| `/api/candidates/` | GET, POST | HR Administrator: C, R; HR Officer: R | HR Administrator: full; HR Officer: read | `email` is not unique at this layer either (`docs/05-database-schema.md` §4.7 — a candidate may apply more than once); this endpoint does not attempt to deduplicate candidates by email |
| `/api/candidates/{id}/` | GET, PATCH | HR Administrator: R, U; HR Officer: R | HR Administrator: full; HR Officer: read | |
| `/api/candidates/{id}/applications/` | GET, POST | HR Administrator: C, R; HR Officer: R | HR Administrator: full; HR Officer: read | Maps to `candidate_application` |
| `/api/applications/{id}/interviews/` | GET, POST | HR Administrator: C, R; HR Officer: R | HR Administrator: full; HR Officer: read | `interviewer_employee_id` may reference any employee, not only HR Administrators — HRMS-FR-018 does not restrict who may interview, only who may schedule |
| `/api/applications/{id}/interviews/{interview_id}/` | PATCH | HR Administrator: U | HR Administrator: full | |
| `/api/applications/{id}/offer/` | GET, POST | HR Administrator: C, R; HR Officer: R | HR Administrator: full; HR Officer: read | Creates `offer_letter`. Body accepts `offered_salary` and optional `offered_pay_grade` (FK into `pay_grade`). `document_object_key` is generated server-side once the offer is issued, same object-storage pattern as employee documents. HR Officer's read access exists so they can reach an accepted offer to trigger conversion (module 9) |
| `/api/offers/{id}/decide/` | POST | HR Administrator: U (records the candidate's decision on their behalf, since a candidate has no account — `CONTEXT.md`: "a candidate is not an employee," and this system has no candidate-facing portal per SRS scope) | n/a | Body: `{"decision": "accepted" \| "rejected" \| "withdrawn"}`. An `accepted` decision does not itself create an employee record — HRMS-BR-013 requires onboarding initiation as the conversion trigger, which is module 9's endpoint below, not this one |
...
| `/api/onboarding-checklists/` | GET | HR Officer: R; HR Administrator: R (`docs/07-iam-rbac.md` §4.2, absorbed from Recruiter — ADR-0013) | Per role, same scoping as Employee Management's read | Filterable: `employee_id`, `application_id`. `POST /api/onboarding/convert/` returns the checklist id at creation time, but that was the only way to learn it (issue #101) — this endpoint lets a caller who only has an employee or application id look the checklist back up |
| `/api/onboarding-checklists/{id}/` | GET | HR Officer: R; HR Administrator: R (`docs/07-iam-rbac.md` §4.2, absorbed from Recruiter — ADR-0013) | Per role, same scoping as Employee Management's read | |
...
| `/api/roles/` | GET | System Administrator (added 2026-07-27, Access redesign) | System Administrator only | Lists the six assigned-role groups (`auth_group.id`, `name`) so the frontend's Grant Access picker doesn't hardcode database-assigned group ids. Read-only reference data, not a `user_account`/`role_grant_request` row |
...
| `/api/pay-grades/` | GET, POST | HR Administrator: C, R, U; HR Officer, Payroll Officer: R | Organisation-wide | Recruiter's blind-selection carve-out (id/name only, no salary figures — added v1.6 for issue #98) is retired with the role, ADR-0013: HR Administrator absorbs Recruiter's access and already has full read here, so the narrower view is dropped rather than reinstated for it |
...
- **`iam.approve_role_grant` holder.** Resolved 2026-07-27 — defaults to HR Administrator by migration (`docs/07-iam-rbac.md` §8). `/api/role-grant-requests/{id}/decide/` (§4.9) enforces the approver-not-requester constraint regardless of who holds the permission.
- **Second-factor recovery approver holder.** Deferred to deployment (`docs/07-iam-rbac.md` §8). `/api/auth/second-factor/recovery-requests/{id}/decide/` (§4.2) functions identically regardless of who is designated.
