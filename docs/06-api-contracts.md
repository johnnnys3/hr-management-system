...
| `/api/applications/{id}/offer/` | GET, POST | Recruiter: C, R; HR Officer: R | Recruiter: full; HR Officer: read | Creates `offer_letter`. `document_object_key` is generated server-side once the offer is issued, same object-storage pattern as employee documents. HR Officer's read access exists so they can reach an accepted offer to trigger conversion (module 9) |
