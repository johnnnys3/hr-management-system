...
| `/api/pay-grades/` | GET, POST, PATCH | HR Administrator: C, R, U; HR Officer, Payroll Officer: R; Recruiter: R (blind selection only) | Organisation-wide | Recruiter's `GET` returns `id`/`name` only, no `min_salary`/`max_salary` — needed to populate `offered_pay_grade` on §4.6's Issue Offer form without granting the salary-figure visibility `docs/07-iam-rbac.md` §2.4 withholds from Recruiter |
